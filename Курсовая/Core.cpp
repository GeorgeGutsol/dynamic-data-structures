...
void showTreeVosr(Node*&Tree, s_Analyz* a)//рекрусивный метод обхода дерева
{
	if (Tree != NULL)//проверяем что ветвь существует
	{
		...
		Dob_Analyz(a, Tree->x->data);//добавляем элемнт в список
		showTree(Tree->L, a);//идем в левую ветвь
	}
}

void showTree(Node*&Tree, s_Analyz* a)//метод обхода дерева
{
	if (Tree != NULL)
	{
		showTree(Tree->L,a);
...
void Analyzirovat(s_Posechaemost* pos, int audit,int dd, int mm, int time, s_Analyz* analyz)//метод для анализа
{
	...
	s_Analyz* an = new s_Analyz;//создаем новый указатель для воспомогательного списка
	an->next = analyz->next;//инициализруем его
	analyz->next = an;//
	an->data.dd = dd;//заполняем данные
	an->data.mm = mm;//
	an->data.nomer_a = audit;//
	an->data.time = time;//
	int sum = 0;//
	int teor = 0;//
	while (pos != p)//цикл
	{
		...
		{
			an->data.den = pos->link_r->data.den;//заполняем данные
			sum +=pos->data.kolvo;//
			teor = pos->link_r->link_a->data.vmestimost;//
		}
		pos = pos->next;//смещаем бегунок
	}
	an->data.zagr = (sum*100) /teor;//ищем загруженность в процентах
}
